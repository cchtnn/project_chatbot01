...
Fixes all 8 failing PDFs with intelligent fallback chains.
...
class DocumentParser:
    ...
    def _parse_pdf_via_ocr(
        ...
        Was broken: reader.readtext(pdf_file_path) - EasyOCR can't read PDFs!
        Fixed: convert PDF to images, then pass images to OCR.
        """
        ...
        try:
            ...
            if reader:
                try:
                    for page_num, image in enumerate(images, 1):
                        results = reader.readtext(image)  # NOW passing Image object, not PDF path!
