...
class StudentTranscriptCSVHandler:
    ...
    def reload_csv(self, new_csv_path: str = None):
        ...
        try:
            ...
            print(f"🔄 Reloading CSV from: {self.csv_path}")
            ...
            if 'GPA' in self.df.columns:
                ...
                print(f"✅ GPA column re-cleaned after reload")
            ...
            print(f"✅ CSV reloaded successfully: {self.df.shape[0]} rows, {self.df.shape[1]} columns")
    ...
    if os.path.exists(csv_path):
        ...
        if cached_handler and hasattr(cached_handler, '_csv_mtime'):
            if current_mtime > cached_handler._csv_mtime:
                file_modified = True
                print(f"📝 CSV file modified, will reload: {csv_path}")
